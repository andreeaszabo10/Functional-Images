...
showRegionAST level (R region) = case region of
    FromPoints points -> indent ++ "FromPoints " ++ show points
    Rectangle width height -> indent ++ "Rectangle " ++ show width ++ " " ++ show height
    Circle radius -> indent ++ "Circle " ++ show radius
    Complement subRegion -> indent ++ "~\n" ++ showRegionAST (level + 1) subRegion
    Union region1 region2 -> indent ++ "+\n" ++
                              showRegionAST (level + 1) region1 ++ "\n" ++
                              showRegionAST (level + 1) region2
    Intersection region1 region2 -> indent ++ "*\n" ++
                                     showRegionAST (level + 1) region1 ++ "\n" ++
                                     showRegionAST (level + 1) region2
    Transform transformation subRegion -> indent ++ show transformation ++ "\n" ++
                                           showRegionAST (level + 1) subRegion
  where
...
instance Functor RegionShape where
    ...
    fmap f region = case region of
        ...
        Circle r -> Circle r
        Complement subRegion -> Complement (f subRegion)
        Union region1 region2 -> Union (f region1) (f region2)
        Intersection region1 region2 -> Intersection (f region1) (f region2)
        Transform transformation subRegion -> Transform transformation (f subRegion)
...
basicEntityCount = foldRegionAST combiner
  where
    ...
    combiner region = case region of
        FromPoints _ -> (1, 0)    -- Counting one basic region
        Rectangle _ _ -> (1, 0)   -- Counting one basic region
        Circle _ -> (1, 0)        -- Counting one basic region
        Complement r -> r         -- Passing counts through
        Union r1 r2 -> combineCounts r1 r2  -- Accumulating counts
        Intersection r1 r2 -> combineCounts r1 r2  -- Accumulating counts
        Transform _ r -> r        -- Passing counts through
