...
toTransformation (Combine transformations) = combineTransformations (reverse transformations)
  where
    combineTransformations [] = id
    combineTransformations (t:ts) = toTransformation t . combineTransformations ts
...
fuseTransformations = foldr fuse []
    where
        fuse (Translation dx1 dy1) (Translation dx2 dy2 : ts) = Translation (dx1 + dx2) (dy1 + dy2) : ts
        fuse (Scaling s1) (Scaling s2 : ts) = Scaling (s1 * s2) : ts
        fuse t ts = t : ts
...
optimizeTransformations = undefined