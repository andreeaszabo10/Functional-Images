...
plot width height region =
    let maxY = fromIntegral height
        minY = fromIntegral (-height)
        maxX = fromIntegral width
        minX = fromIntegral (-width)
        plotOne x y = if (x, y) `inside` region then '*' else '.'
        plotRow y = [plotOne x y | x <- [minX..maxX]]
        plotAll = intercalate "\n" [plotRow y | y <- [maxY, (maxY - 1)..minY]]
    in plotAll
...
bfs start expand = helper [(start, 0)] [start]
  where
    helper ((node, level) : queue) visited = (node, level) : helper newQ (visited ++ map fst newQ)
      where
        neighbours = filter (`notElem` visited) $ expand node
        newQ = queue ++ [(neighbour, level + 1) | neighbour <- neighbours]
...
regionAvoidingBfs start region = bfs start expand
  where
    expand :: Point -> [Point]
    expand point = filter (\neighbour -> not (inside neighbour region) && S.notMember neighbour visited) $ neighbours point
    visited = S.empty